...
export class DistributionManager implements DistributionProvider {
  ...
  constructor(
    ...
    this.extensionSpecificDistributionManager = new ExtensionSpecificDistributionManager(
      config,
      versionRange,
      extensionContext
    );
  ...
  public async getDistribution(): Promise<FindDistributionResult> {
    ...
    const includePrerelease =
      distribution.kind !== DistributionKind.ExtensionManaged || this.config.includePrerelease;
  ...
  public async checkForUpdatesToExtensionManagedDistribution(
    ...
    const updateCheckResult = await this.updateCheckRateLimiter.invokeFunctionIfIntervalElapsed(
...
class ExtensionSpecificDistributionManager {
  ...
  private async downloadDistribution(
    ...
    const assetStream = await this.createReleasesApiConsumer().streamBinaryContentOfAsset(
      assets[0]
    );
    ...
    try {
      ...
      await new Promise((resolve, reject) =>
        assetStream.body.pipe(archiveFile).on('finish', resolve).on('error', reject)
      );
