...
describe('EvalLogTreeBuilder', () => {
  it('should build the log tree roots', async () => {
    ...
    expect(
      roots[0].children.map((pred) => ({ ...pred, children: undefined, parent: undefined }))
    ).to.deep.eq(expectedPredicate);

    expect(
      roots[0].children[0].children.map((ra) => ({ ...ra, children: undefined, parent: undefined }))
    ).to.deep.eq(expectedRA);

    // Pipeline steps' children should be empty so do not force undefined children here.
    expect(
      roots[0].children[0].children[0].children.map((step) => ({ ...step, parent: undefined }))
  ...
  it('should build the tree with descriptive message when no data exists', async () => {
    ...
    expect(roots[0].children.map((noPreds) => ({ ...noPreds, parent: undefined }))).to.deep.eq(
      expectedNoPredicates
    );
  });
});
