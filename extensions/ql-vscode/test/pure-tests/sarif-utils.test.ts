...
describe('parsing sarif', () => {
  ...
  it('should be able to parse a broken complex message from the spec', async function () {
    const message = 'Prohibited term used in [para\\[0\\]\\\\spans\\[2\\](1).';
    const results = parseSarifPlainTextMessage(message);
    expect(results).to.deep.equal(['Prohibited term used in [para[0]\\spans[2](1).']);
  });
  it('should be able to parse a message with extra escaping the spec', async function () {
    const message = 'Tainted data was used. The data came from \\[here](3).';
    const results = parseSarifPlainTextMessage(message);
    expect(results).to.deep.equal(['Tainted data was used. The data came from [here](3).']);
  });
  ...
  it('should normalize source locations', () => {
    ...
    expect(getPathRelativeToSourceLocationPrefix('C:\\a\\b', '%3Fx%3Dtest')).to.eq(
      'file:/C:/a/b/%3Fx%3Dtest'
    );
    expect(getPathRelativeToSourceLocationPrefix('C:\\a =\\b c?', '?x=test')).to.eq(
      'file:/C:/a%20%3D/b%20c%3F/?x=test'
    );
    expect(getPathRelativeToSourceLocationPrefix('/a/b/c', '?x=test')).to.eq('file:/a/b/c/?x=test');
  });
